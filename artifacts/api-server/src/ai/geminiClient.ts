...
/**
 * ===================================================================
 * RAG-POWERED AI FUNCTIONS (using textbook chunks instead of File Search)
 * ===================================================================
 */

import { searchTextbookChunks, type SearchResult } from "@workspace/textbooks";
...
/**
 * Explain a concept using RAG retrieval from textbook chunks
 */
export async function explainFromBookRAG(
  subjectId: number,
  questionText: string,
  opts: { language?: ReplyLanguage } = {}
...
Explanation:`;
  ...
  const explanation = (response as { text?: string }).text || "";
...
/**
 * Chat with the textbook using RAG retrieval
 */
export async function chatWithBookRAG(
  subjectId: number,
  messages: Array<{ role: "user" | "assistant"; content: string }>,
  opts: ChatStreamOptions = {}
  ...
  const tutorInstruction =
    mode === "tutor"
      ? "You are a Socratic tutor. Ask guiding questions instead of giving direct answers. Only reveal the full answer if the student explicitly asks or is stuck."
      : "";
 ...
 */
export async function* streamChatWithBookRAG(
  subjectId: number,
  messages: Array<{ role: "user" | "assistant"; content: string }>,
  opts: ChatStreamOptions = {}
): AsyncGenerator<{ type: "text"; text: string } | { type: "done"; citations: RAGCitation[] }> {
  const client = getGeminiClient();
  const mode = opts.mode ?? "answer";
  const lang = languageInstruction(opts.language);
  ...
  const tutorInstruction =
    mode === "tutor"
      ? "You are a Socratic tutor. Ask guiding questions instead of giving direct answers."
      : "";
...
/**
 * Stream explanation using RAG
 */
export async function* streamExplainFromBookRAG(
  subjectId: number,
  questionText: string,
  opts: { language?: ReplyLanguage } = {}
): AsyncGenerator<{ type: "text"; text: string } | { type: "done"; explanation: string; citations: RAGCitation[] }> {
  const client = getGeminiClient();
  const lang = languageInstruction(opts.language);
...
/**
 * Generate questions using RAG-retrieved context
 */
export async function generateQuestionsRAG(
  subjectId: number,
  topicText: string,
  questionType: "mcq" | "short" | "long",
  count: number,
  ...
  const difficultyText =
    opts.difficulty === "harder"
      ? "Make these questions HARDER: combine concepts, require reasoning."
      : opts.difficulty === "easier"
        ? "Make these questions EASIER: focus on recall and basic concepts."
        : "Match typical board exam difficulty.";

  const prompt = `${lang}${difficultyText}

Generate ${count} ${questionType} questions based on this textbook content:

${context}

Return as JSON array with this format:
[{"question": "...", "options": {"A": "...", "B": "...", "C": "...", "D": "..."}, "correctOption": "A", "sourcePage": 10, "explanation": "..."}]
